...
- `explore` (generator, `Read Write Edit Bash`): produces `note.tex` (LaTeX, `amsart` class) and `result.json` (claims with `label` ∈ `{theorem-complete-proof, theorem-sketch, conjecture-with-evidence, computation, speculation}` and `proof_status` ∈ `{unchecked, verified, gap-found, counterexample-found}`). Gated by `compile_latex()` and `validate_result_json()` before advancing.
...
- *Transient* (429/session-limit/overload/empty credit balance, detected by `is_transient_failure()`): `defer_stage()` leaves the run at its current stage with **no attempt consumed**; a later invocation retries. Never archive for these.
